...
/*   Updated: 2025/04/08 18:05:39 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	push_chunk_to_b(t_stack **a, t_stack **b, int chunk_start, int chunk_end)
{
    ...
    while (pushed < target)
    {
        ...
        if ((*a)->index >= chunk_start && (*a)->index <= chunk_end)
        {
            ...
            pushed++;
            if (stack_size(*b) > 1
                && (*b)->index < (chunk_start + chunk_end) / 2)
                rb(b);
        }
        else
        {
...
void	turk_sort(t_stack **a, t_stack **b)
{
	...
	if (!sorted)
		return ;        /* <<< ajouté : on arrête si OOM */
	...
	while (i < size)
	{
		...
		if (chunk_end >= size)
			chunk_end = size - 1;
		push_chunk_to_b(a, b, sorted[chunk_start], sorted[chunk_end]);
		i += chunk_size;
	}
	free(sorted);
	push_back_to_a(a, b);
